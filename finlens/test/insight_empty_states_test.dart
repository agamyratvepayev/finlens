...
void main() {
  ...
  group('state selection and header buttons', () {
    ...
    testWidgets('state 1 — no accounts: no filter, no eye, add + body',
        (tester) async {
      ...
      expect(find.text(l.moreAddAccount), findsOneWidget);
    });

    testWidgets('state 2 — no records: eye yes, filter no, holdings line',
        (tester) async {
      await pump(tester, accountsNoTxns());
      final l = await AppLocalizations.delegate.load(const Locale('en'));
      expect(find.text(l.insEmptyNoRecordsTitle), findsOneWidget);
    ...
    testWidgets('state 3 — everything hidden: filter filled, names the count',
        (tester) async {
  ...
  // ── Widget — the actions ────────────────────────────────────────────────────
  group('actions', () {
    testWidgets('Add an account opens showNewAccountSheet', (tester) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      ...
      final l = await AppLocalizations.delegate.load(const Locale('en'));
      await tester.tap(find.text(l.moreAddAccount));
      await tester.pumpAndSettle();
      // The New account sheet's title appears.
      expect(find.text(l.qaNewAccount), findsOneWidget);
    });

    testWidgets('Record something opens Quick Add', (tester) async {
      tester.view.physicalSize = const Size(390, 844);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.reset);
      await tester.pumpWidget(app(accountsNoTxns()));
      await tester.pump(const Duration(milliseconds: 300));

      final l = await AppLocalizations.delegate.load(const Locale('en'));
      await tester.tap(find.text(l.insEmptyRecordSomething));
      await tester.pumpAndSettle();
