...
/// The account & group filter sheet, shared by Balance and Insight (spec §9).
///
...
///
/// [selector] reads the caller's filter from the store and [onApply] writes it
/// back, so the sheet stays fully store-driven (it subscribes and rebuilds on
/// every apply). Balance passes its own `balanceFilter`/`setBalanceFilter`
/// pair, Insight its independent one — the behaviour, copy, ordering and counts
/// are identical to the old Balance-only sheet whichever slot is wired.
Future<void> showAccountFilterSheet(
  ...
  return showModalBottomSheet<void>(
    ...
    builder: (_) =>
        _AccountFilterSheet(selector: selector, onApply: onApply),
  );
}

class _AccountFilterSheet extends StatefulWidget {
  const _AccountFilterSheet({required this.selector, required this.onApply});

  final BalanceFilter Function(AppStore) selector;
  final void Function(AppStore, BalanceFilter) onApply;
...
class _AccountFilterSheetState extends State<_AccountFilterSheet> {
  ...
  void _apply(BalanceFilter next) =>
      widget.onApply(StoreScope.read(context), next);
  ...
  Widget build(BuildContext context) {
    ...
    final filter = widget.selector(store);
